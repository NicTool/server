...
/**
 * NameserverSupervisor – starts/stops the set of nameserver engines declared
 * in nictool.toml. Built to support many instances (so the deployment can run
 * several native NSes side-by-side, or mix native with export engines).
 *
 * The top-level [store] block is the default Source for every engine; each
 * [[nameserver]] may override via [nameserver.source].
 *
 * Emits:
...
export class NameserverSupervisor extends EventEmitter {
  ...
  async start(nicConfig) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        ...
        console.log(
          `Nameserver[${engine.name ?? i}] started: engine=${engine.engine} listen=${JSON.stringify(engine.listen)}`,
        )
  ...
  _build(nsCfg, nicConfig, idx) {
    const engineName = nsCfg.engine ?? nsCfg.type ?? (nsCfg.exportType ? 'bind' : 'native')
    const source = this._buildSource(nsCfg, nicConfig)

    // Normalize listen[] from either the richer form or the legacy {host,port} shape.
    const listen = Array.isArray(nsCfg.listen) && nsCfg.listen.length
      ? nsCfg.listen
    ...
    const publisherCfg = nsCfg.publisher ?? { type: engineName === 'native' ? 'memory' : 'rfc1035' }

    const signer = this._buildSigner(nsCfg)

    if (engineName === 'native') {
      if (publisherCfg.type !== 'memory') {
        throw new Error(`native engine requires publisher.type=memory (got "${publisherCfg.type}")`)
      }
      return new NativeNS({
        id: nsCfg.id ?? idx + 1,
        name: nsCfg.name,
  ...
  _buildPublisher(cfg, engineName) {
    const type = cfg?.type ?? (engineName === 'tinydns' ? 'tinydns-cdb' : 'rfc1035')
    switch (type) {
      case 'rfc1035':
        return new Rfc1035Publisher({ path: cfg?.path })
      case 'tinydns-cdb':
        return new TinydnsCdbPublisher({ path: cfg?.path })
      case 'powerdns-db':
  ...
  _buildTransport(cfg) {
    ...
    switch (type) {
      ...
      case 'rsync':
        return new RsyncTransport({ remote: cfg.remote, sshKey: cfg.sshKey, interval, cooldown })
      case 'axfr':
        return new AxfrTransport({ master: cfg.master, tsigKey: cfg.tsigKey, interval, cooldown })
      case 'db-replication':
  ...
  _buildSource(nsCfg, nicConfig) {
    ...
    switch (type) {
      case 'directory':
      case 'toml':
        return new TomlSource({ path: srcCfg.path })
      case 'mysql':
