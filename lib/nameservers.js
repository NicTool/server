...
export class NameserverSupervisor extends EventEmitter {
  ...
  async start(nicConfig) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      let ns = null
      try {
        ns = this._build(nsCfg, nicConfig, i)
        ns.on('error', (err) => this._emitError(ns, err))
        await ns.start()
        ...
        // start() connects the source before the publish that failed, and a
        // FileSource watcher is deliberately not unref'd. Nothing else holds
        // this one — it never reached this.nameservers — so without an explicit
        // stop it keeps the event loop alive for good.
        await this._stop(ns)
        this._emitError(null, err)
      }
    }
  }

  // EventEmitter throws when 'error' has no listener, which turned one
  // nameserver failing to start into every later one never being tried.
  _emitError(ns, err) {
    if (this.listenerCount('error')) this.emit('error', ns, err)
  }
  ...
  async stop() {
    for (const ns of this.nameservers) {
      if (await this._stop(ns)) this.emit('stopped', ns)
    }
    this.nameservers = []
  }
