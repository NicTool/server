...
`
  ...
  it('signs zone files for a nameserver that reads them', async (t) => {
    ...
    const sup2 = new NameserverSupervisor()
    const errors = []
    sup2.on('error', (_ns, err) => errors.push(err))
    try {
      await sup2.start({
        store: { type: 'directory', path: storeDir },
        nameserver: [
      ...
      const noTool = errors.find((err) => /not found/.test(err.message))
      if (noTool) return t.skip(noTool.message)
      assert.equal(sup2.status().length, 1, errors[0]?.message ?? 'signed. should run')
